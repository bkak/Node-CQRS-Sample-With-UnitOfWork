...
module.exports = {
    ...
    emitException: function(error) {
        console.log(colors.red('\nhub -- publishing exception ' + error.name + ' to redis:'));
        console.log(error.message);
        err.publish('exception', JSON.stringify(error));
    },
    onException: function(callback) {
        if (errSubscriptions.length === 0) {
            // subscribe to __events channel__
            err.subscribe('exception');
        }
        errSubscriptions.push(callback);
        console.log(colors.red('hub -- exception subscribers: ' + errSubscriptions.length));
    }
};
...
//listen to domainexceptions from redis
err.on('message', function(channel, message) {

    var error = JSON.parse(message);
    if (channel === 'exception') {
        console.log(colors.red('\nhub -- received exception ' + error.name + ' from redis:'));
        console.log(error);

        errSubscriptions.forEach(function(subscriber){
            subscriber(error);
        });
    }
});
