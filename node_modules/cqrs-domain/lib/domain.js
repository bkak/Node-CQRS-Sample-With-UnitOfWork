...
module.exports = domain = _.extend(new EventEmitter2({
    ...
    initialize: function(options, newGetCommandId, callback) {
        ...
        eventEmitter.on('DomainExceptionInternal' ,  function(err){
            console.log('Emit Domain Exception');
            domain.emit('DomainException', err);
        });
        async.parallel({
                            ...
                            var process = function(cmdPointer, next) {
                                ...
                                async.waterfall([
                                    ...
                                    function( id, callback){
                                        cmdPointer.command.CorelationId  = id;
                                        var uow = new UOW();
                                        uow.Initialize(id, cmdPointer.command, commandDispatcher, function(){
