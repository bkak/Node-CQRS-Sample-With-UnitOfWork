    ...
    reEmitCommands: function(callback) {
        this.commandQueue.getAll(function(err, cmds) {
            async.forEach(cmds, function(item, cb) {
                ...
                eventEmitter.emit('command:' + item.data.command,  item.data);//, item.id, item.data);
                //will be subscribed by domain and it will be assigned new uow and the cycle continues.
                cb();
            }, callback);
        });
    },

    dispatch: function(uow, cmd, callback) {
            ...
            function(id, callback) {
                commandQueue.push(id, cmd, function(err) {
                    if (!err) {
                        ...
                        eventEmitter.emit('handle:' + cmd.command, uow.CorelationId, cmd, uow);
                    } else {
                        //eventEmitter.emit('commandRejected', cmd, err.message);
                        eventEmitter.emit('commandRejected'+ uow.CorelationId, uow.CorelationId, cmd, uow);
                    }
